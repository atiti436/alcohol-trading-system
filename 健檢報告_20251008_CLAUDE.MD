...
|---------|---------|---------|---------|------|
| 問題 5 | API 路由版本統一（imports v2 遷移） | 1-2 天 | 前後端整合 | 待處理 |
| 問題 6 | 出貨 API 變數未定義錯誤 | - | 前後端整合 | ✅ 已修復（2025-10-08） |
| 問題 8 | 完成報表匯出功能（Excel） | - | 報表功能 | ✅ 已完成（2025-10-08） |
| 問題 11 | 重新啟用 CORS 檢查 | 0.5 天 | 安全 | 待處理 |

**小計**: 4-4.5 天（剩餘 1.5-2.5 天，問題 6、8 已完成）
...
| 問題編號 | 問題描述 | 預估工時 | 負責模組 |
|---------|---------|---------|---------|
| 問題 1 | UI 樣式一致性統一 | 2-3 天 | UI/UX |
| 問題 2 | 間距系統統一 | 1-2 天 | UI/UX |
| 問題 9 | 實作報表自動刷新 | 0.5 天 | 報表功能 |
| 問題 12 | 完善 Token 過期機制 | 1 天 | 安全 |
| 問題 13 | 實作操作日誌 | 2-3 天 | 安全 |
| 問題 14 | 清理技術債務 | 1-2 天 | 架構 |
...
3. 完成 imports v2 遷移（補齊 PATCH、costs、finalize）
4. 重新啟用 CORS 檢查

**短期執行（1 個月）**:
1. 統一 UI 設計系統
2. 清理技術債務（@deprecated 欄位）
3. 完善安全機制（Token、操作日誌）
