...
#### 1.1 數據庫 Schema 修改
- [ ] 擴充 `SaleStatus` enum（加入 PREORDER, PARTIALLY_CONFIRMED, BACKORDER）
- [ ] `Sale` 表新增預購相關欄位
...
#### 1.2 預購訂單基礎 API
- [ ] `POST /api/sales` - 修改支援預購模式（`is_preorder: true`）
- [ ] `POST /api/sales/[id]/confirm` - 修改邏輯：預購單跳過庫存檢查
- [ ] `GET /api/sales` - 修改支援預購篩選
...
- `webapp/src/app/sales/[id]/page.tsx`
