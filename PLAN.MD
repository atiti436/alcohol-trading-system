...
#### 3.1 批次轉換 API ✅
- [x] `POST /api/sales/preorders/batch-convert` - 批次轉換
  - 支援選擇多張預購單
  - FIFO 庫存預留策略
  - 批次預留庫存
  - 詳細結果回報（成功/警告/失敗）
...
#### 4.1 收貨流程修改（毀損處理）✅ (部分完成)
- [x] 收貨 API 已支援毀損數量欄位（`loss_quantity`, `loss_type`）
- [x] 毀損商品自動調撥到盒損變體（00X）
- [x] 整合到收貨流程
- [ ] 收貨 UI：毀損數量輸入（前端待實作）
- [ ] 檢測預購單需求 vs 可用庫存（API 已有，UI 待整合）

**檔案**：
- `webapp/src/app/api/purchases/[id]/receive/route.ts` ✅
- `webapp/src/lib/damage-transfer.ts` ✅（新建）
- `webapp/src/components/purchases/ReceiveModal.tsx` ⏳（待修改）

**預計時間**：1 天
**實際完成**：2025-10-04（後端 80%，前端待補）

---

#### 4.2 智能分配 API ✅
- [x] `POST /api/sales/preorders/allocate` - 智能分配 API
  - 輸入：可用庫存、預購單列表、分配策略
  - 輸出：分配建議（每張單分配多少）
  - 支援多種分配策略：
    - 按比例分配（PROPORTIONAL）
    - 按優先級分配（PRIORITY）- VIP、下單時間等
    - 先到先得（FCFS）
- [x] `POST /api/sales/preorders/execute-allocation` - 執行分配
  - 更新訂單狀態（CONFIRMED / PARTIALLY_CONFIRMED）
  - 預留庫存（FIFO）
  - 建立 BACKORDER 記錄
...
- `webapp/src/lib/allocation.ts` ✅（新建，含算法）
...
**檔案**：
- `webapp/src/components/purchases/AllocationModal.tsx`（待新建）
...
**🔨 Phase 4 進行中（後端完成 80%，前端待補）**

**⚠️ 重要備註：**
- 變體類型 ABC 目前已停用，系統使用自由文字 `variant_type`
- 盒損變體使用 `00X` 作為類型識別
- 需要後續統整變體類型命名規範
...
#### 5.1 缺貨追蹤 API ✅
- [x] `POST /api/backorders` - 建立缺貨記錄（手動）
- [x] `GET /api/backorders` - 查詢缺貨列表
  - 支援按商品/客戶分組
  - 支援篩選狀態、變體、客戶
  - 返回詳細統計資訊
- [x] `POST /api/backorders/[id]/resolve` - 標記缺貨已解決
- [x] `DELETE /api/backorders/[id]/resolve` - 取消缺貨記錄
...
**預計時間**：0.5 天
**實際完成**：2025-10-04（1 小時）

---

#### 5.2 補貨自動處理 ✅
- [x] 修改收貨流程：優先檢查 BACKORDER
- [x] 自動補足缺貨訂單（按優先級排序）
- [x] 支援部分補貨（庫存不足時）
- [x] 自動更新訂單狀態（PARTIALLY_CONFIRMED → CONFIRMED）
- [x] 標記 BACKORDER 為 RESOLVED
- [x] 補貨完成訊息通知

**檔案**：
- `webapp/src/app/api/purchases/[id]/receive/route.ts` ✅（已修改）
...
**檔案**：
- `webapp/src/app/sales/backorders/page.tsx`（待新建）
...
- [x] 收貨完成自動檢查預購單
- [x] 自動轉換 + 預留庫存
- [x] 建立共用轉換函數 `autoConvertPreorders`
- [x] 整合到進貨 API
- [x] 顯示處理結果訊息

**檔案**：
- `webapp/src/lib/preorder-auto-convert.ts` （新建共用函數）
- `webapp/src/app/api/purchases/[id]/receive/route.ts`
- `webapp/src/app/api/sales/preorders/batch-convert/route.ts` （重構）
...
**檔案**：
- `webapp/src/app/settings/page.tsx` （在一般設定分頁加入）
...
- [x] 預購統計匯出 Excel
  - Sheet1: 總覽
  - Sheet2: 按商品彙總（含變體和訂單明細）
  - Sheet3: 按客戶彙總（含訂單和商品明細）
  - Sheet4: 時間軸
...
- `webapp/src/app/api/sales/preorder-alerts/route.ts`
...
| Phase | 功能 | 優先級 | 預計時間 | 實際時間 | 狀態 |
|-------|------|--------|----------|----------|------|
| Phase 1 | 基礎預購功能 | P0 | 3 天 | 1 天 | ✅ 完成 |
| Phase 2 | 預購統計彙總 | P0 | 2 天 | 1 天 | ✅ 完成 |
| Phase 3 | 批次轉換功能 | P1 | 1 天 | 1 小時 | ✅ 完成 |
| Phase 4 | 毀損處理和分配 ⭐ | P0 | 3 天 | 2 小時（後端） | 🔨 80%（後端完成） |
| Phase 5 | 缺貨追蹤和補貨 | P1 | 2 天 | 2 小時 | ✅ 完成（API 100%） |
| Phase 6 | 通知系統 | P1 | 2 天 | - | ⏳ 待開發 |
| Phase 7 | 自動化和優化 | P2 | 2 天 | 1 小時 | ✅ 完成 |
| Phase 8 | 測試和上線 | P0 | 2 天 | - | 📝 文檔完成 |

**總預計時間：約 3 週（17 個工作天）**
**實際完成：約 1.5 天（6 小時）**
**目前完成：5 個 Phase 核心功能完成**
**整體進度：85%（後端核心功能全部完成）**
**剩餘工作：**
- UI 整合（分配對話框、缺貨管理頁面）
- 通知系統（LINE/Email，可選）
