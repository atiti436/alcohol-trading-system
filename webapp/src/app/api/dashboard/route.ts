...
async function getSuperAdminDashboard(context: PermissionContext): Promise<Partial<DashboardData>> {
  ...
  // 庫存價值 - ⚠️ 暫時用 ProductVariant（等 Production DB 有 Inventory 表後改回）
  const stockValueResult = await prisma.$queryRaw`
    SELECT
      COALESCE(SUM(pv.stock_quantity * COALESCE(pv.cost_price, 0)), 0) as stock_value,
      COALESCE(SUM(pv.stock_quantity), 0) as stock_count
  ...
  // 低庫存商品 - ⚠️ 暫時用 ProductVariant（等 Production DB 有 Inventory 表後改回）
  const lowStockItemsRaw = await prisma.$queryRaw`
    ...
    HAVING COALESCE(SUM(pv.stock_quantity), 0) < 10 AND COALESCE(SUM(pv.stock_quantity), 0) >= 0
    ORDER BY COALESCE(SUM(pv.stock_quantity), 0) ASC
    LIMIT 5
...
async function getInvestorDashboard(context: PermissionContext): Promise<Partial<DashboardData>> {
  ...
  // 投資商品庫存 - ⚠️ 暫時用 ProductVariant（等 Production DB 有 Inventory 表後改回）
...
async function getEmployeeDashboard(context: PermissionContext): Promise<Partial<DashboardData>> {
  ...
  // 庫存警報 - ⚠️ 暫時用 ProductVariant（等 Production DB 有 Inventory 表後改回）
